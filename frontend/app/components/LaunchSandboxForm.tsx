  ...
  return (
    <div className="flex flex-col gap-10 mt-2 pb-12">
      <div className="flex gap-[45px] items-start">
        <div className="w-[320px] h-[180px] shrink-0 rounded-xl border border-[#363636] bg-card overflow-hidden opacity-60 flex items-center justify-center">
          <span className="text-muted-foreground text-sm font-mono">Preview pending launch...</span>
        </div>
        <div className="flex flex-col py-2">
          <h2 className="text-[20px] font-normal text-foreground m-0 leading-none mb-4 font-sans tracking-tight">Launch sandbox</h2>
          <p className="text-[16px] text-muted-foreground leading-none m-0 font-mono">{presetName}</p>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-8 max-w-[800px]">
        ...
        <div className="card-dev overflow-hidden">
          <button
            ...
            className="w-full flex items-center gap-4 px-5 py-6 hover:bg-secondary/80 transition-colors"
          >
            <Settings size={20} className="text-muted-foreground flex-shrink-0" />
            <span className="text-[18px] font-normal text-foreground font-sans tracking-tight">Deployment Settings</span>
          ...
          {settingsOpen && (
            <div className="px-5 pb-6 pt-2 border-t border-[#363636]">
              {hasConfigSchema && preset ? (
                    ...
                    if (type === "boolean") {
                      return (
                        <div className="flex flex-row items-center gap-3 md:col-span-2" key={key}>
                          <label className="min-w-[120px] text-sm text-foreground font-sans">{label}</label>
                          <input
                            type="checkbox"
                    ...
                    if (type === "select" && field.options?.length) {
                      return (
                        <div className="flex flex-col gap-1.5" key={key}>
                          <label className="text-sm text-foreground font-sans">{label}</label>
                          <select
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm text-foreground font-sans">Config (JSON)</label>
                  <textarea
                    value={configJson}
        ...
        <div className="card-dev overflow-hidden">
          ...
          >
            ...
            <span className="text-[18px] font-normal text-foreground font-sans tracking-tight">Agent Settings</span>
            <div className="ml-auto">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <label className="text-sm text-foreground font-sans">Expires in</label>
            <select
              value={expiresIn ?? ""}
