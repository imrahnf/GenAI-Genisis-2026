...
`backend/agent.py` reads `OLLAMA_MODEL` at startup; all agents share that model. For plan-driven behavior (init once + continuous transfer loop), use e.g. `export OLLAMA_MODEL=qwen2.5-coder:7b`. Set `DEMOFORGE_AGENT_MAX_TOKENS=256` to cap LLM output for faster CPU inference; when the planner succeeds, the continuous phase uses a plan-driven loop (no per-step LLM). To use a remote or local OpenAI-compatible LLM (e.g. `http://localhost:8000/v1` with `model=openai/gpt-oss-20b`, or Hugging Face), set `OPENAI_COMPATIBLE_BASE`, `OPENAI_COMPATIBLE_MODEL`, and optionally `OPENAI_COMPATIBLE_API_KEY` (use `EMPTY` if your server expects it). With the `openai` package installed, the agent uses the same pattern as `OpenAI(base_url=..., api_key=...).chat.completions.create(...)`. See [docs/USE_GUIDE.md](docs/USE_GUIDE.md).
